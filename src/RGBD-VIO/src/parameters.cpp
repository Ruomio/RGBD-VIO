...
T readParam(rclcpp::Node &n, std::string name)
{
    ...
    {
        RCLCPP_INFO_STREAM(rclcpp::get_logger("parameters"), "Loaded " << name << ": " << ans);
    }
    else
    {
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("parameters"), "Failed to load " << name);
        // n.shutdown();
        rclcpp::shutdown();
    }
    return ans;
...
void readParameters(std::shared_ptr<rclcpp::Node> &n)
{
    ...
    if(!fsSettings.isOpened())
    {
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("parameters"), "parameters.cpp: succeed to load config_file: %s", config_file.c_str());
    }
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("parameters"), "VINS_RESULT_PATH: %s", VINS_RESULT_PATH.c_str());
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("parameters"), "SOLVER_TIME: %lf PIX_SIGMA = %lf ACC_N = %lf ACC_W = %lf GYR_N = %lf GYR_W = %lf G.z = %lf", SOLVER_TIME, PIX_SIGMA, ACC_N, ACC_W, GYR_N, GYR_W, G.z());
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("parameters"), "MAX_DEPTH_RANGE: %lf DPT_VALID_RANGE: %lf normal g: %lf", MAX_DPT_RANGE, DPT_VALID_RANGE, nG);
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("parameters"), "FX = %f FY = %f CX = %f CY = %f", FX, FY, CX, CY);
...
/*
        ...
        RCLCPP_INFO_STREAM("before normalize Extrinsic_R : " << std::endl << RIC[0]);
        Eigen::Quaterniond Q(RIC[0]);
        RIC[0] = Q.normalized();
        // RCLCPP_INFO_STREAM(" Q : " << std::endl << Q.w()<<" "<<Q.x()<<" "<<Q.y()<<" "<<Q.z());
        TIC.push_back(T.block<3,1>(0,3));
        RCLCPP_INFO_STREAM("Extrinsic_R : " << std::endl << RIC[0]);
        RCLCPP_INFO_STREAM("Extrinsic_T : " << std::endl << TIC[0].transpose());
        ...
        if(NUM_OF_CAM == 2){
            ...
            RIC[1] = Q.normalized();
            TIC.push_back(T.block<3,1>(0,3));
            RCLCPP_INFO_STREAM("cam2 Extrinsic_R : " << std::endl << RIC[1]);
            RCLCPP_INFO_STREAM("cam2 Extrinsic_T : " << std::endl << TIC[1].transpose());
        }
*/
