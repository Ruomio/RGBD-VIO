...
double FeaturePerId::parallax_angle(Matrix3d Rs[], Matrix3d& ric, double * parallax)
{
    ...
    for(int i=0; i<(int)feature_per_frame.size(); i++){
...
int FeatureManager::getFeatureCount()
{
    ...
    for (auto &it : feature)
    {
        ...
        if(it.used_num >= 2 && it.start_frame < WN - 2)
        {
            cnt++;
        }
    }
...
void FeatureManager::removeBackShiftDepth_dvio(Eigen::Matrix3d marg_R, Eigen::Vector3d marg_P, Eigen::Matrix3d new_R, Eigen::Vector3d new_P)
{
         ...
         it != feature.end(); it = it_next)
    {
        ...
        else
        {
            ...
            double feature_depth = it->estimated_depth;
            it->feature_per_frame.erase(it->feature_per_frame.begin());
                ...
                if(it->depth_shift == 0){
                    ...
                    if(min_d > 0 && shift >= 0){
                        ...
                        if(it->feature_per_frame[shift].lambda > 0 &&  it->feature_per_frame[shift].sig_l > 0)
                            ...
                            it->estimated_depth = it->feature_per_frame[shift].dpt;
                        it->dpt_type = DEPTH_MES;
                    }else if(it->estimated_depth > 0){ // no depth measurement, just propergate depth
                        Eigen::Vector3d pts_i = uv_i * feature_depth; // it->estimated_depth;
                        Eigen::Vector3d w_pts_i = marg_R * pts_i + marg_P;
                        Eigen::Vector3d pts_j = new_R.transpose() * (w_pts_i - new_P);
                        double dep_j = pts_j(2);
                        if(dep_j > 0.)
    ...
    RCLCPP_WARN(rclcpp::get_logger("feature_manager"), "feature_manager.cpp: number of feature depth invalid %d, number of feature to be deleted: %d", cnt_invalid_depth, cnt_deleted_feat);
}
...
bool FeatureManager::addFeatureCheckParallaxSigma_dvio(int frame_count, const map<int, vector<pair<int, Eigen::Matrix<double, 10, 1>>>> &image, bool finish_ini)
{
    RCLCPP_DEBUG(rclcpp::get_logger("feature_manager"), "input feature: %d", (int)image.size());
    RCLCPP_WARN(rclcpp::get_logger("feature_manager"), "before addFeatureCheckParallax num of feature: %d total feature num: %d", getFeatureCount(), (int)feature.size());
    ...
    RCLCPP_INFO(rclcpp::get_logger("feature_manager"), "after addFeatureCheckParallax num of feature: %d last_track_num: %d", getFeatureCount(), last_track_num);
...
//     RCLCPP_DEBUG("parallax_sum: %lf, parallax_num: %d", parallax_sum, parallax_num);
//     RCLCPP_DEBUG("current parallax: %lf", parallax_sum / parallax_num * FOCAL_LENGTH);
//         // last_average_parallax = parallax_sum / parallax_num * FOCAL_LENGTH;
//     return ((parallax_sum / parallax_num) >= MIN_PARALLAX);
// }
...
void FeatureManager::triangulate(Vector3d Ps[], Matrix3d Rs[], Vector3d tic[], Matrix3d ric[])
{
    for (auto &it_per_id : feature)
    {
        ...
        RCLCPP_ASSERT(rclcpp::get_logger("feature_manager"), svd_idx == svd_A.rows());
...
double FeatureManager::compensatedParallax2(const FeaturePerId& it_per_id, int frame_count)
{
    ...
    if(abs(ans) >= 10){
        RCLCPP_INFO(rclcpp::get_logger("feature_manager"), "ans = %lf, feature_id %d du = %lf dv = %lf  dep_i = %lf u_i = %lf vi = %lf u_j = %lf v_j = %lf",
