...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (cur_pts.size() > 0)
    {
        ...
        // RCLCPP_INFO("KLT track %d features", forw_pts.size());
...
map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> FeatureTracker::trackImage(double _cur_time, const cv::Mat &_img, const cv::Mat &_img1)
{
    ...
    */
    ...
    if (prev_pts.size() > 0)
    {
        ...
        reduceVector(track_cnt, status);
        // RCLCPP_DEBUG("temporal optical flow costs: %fms", t_o.toc());
        RCLCPP_ERROR(rclcpp::get_logger("feature_tracker"), "feature_tracker.cpp: timestamp %lf track cnt %d\n", _cur_time,  (int)ids.size());
    }
    ...
    if (1)
    {
        ...
        if (n_max_cnt > 0)
        {
            ...
            n_pts.clear();
        // RCLCPP_DEBUG("detect feature costs: %f ms", t_t.toc());
...
void FeatureTracker::readIntrinsicParameter(const vector<string> &calib_file)
{
    ...
    {
        // RCLCPP_INFO("reading paramerter of camera %s", calib_file[i].c_str());
