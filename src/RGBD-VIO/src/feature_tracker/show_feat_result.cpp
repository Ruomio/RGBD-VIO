...
int main(int argc, char* argv[])
{
	...
	auto node = std::make_shared<rclcpp::Node>("show_feat_result");

    // ros::NodeHandle n("~");
    // ros::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, ros::console::levels::Info);  // Debug Info

    if(argc < 3){
    	RCLCPP_INFO(node->get_logger(), "usage: ./show_feat_result [rgb] [dpt]");
    	return -1;
    }
    ...
    if(bgr.empty()){
    	RCLCPP_ERROR(node->get_logger(), "show_feat_result: failed to load rgb file %s", argv[1]);
    	return -1;
    }
    if(dpt.empty()){
    	RCLCPP_ERROR(node->get_logger(), "show_feat_result: failed to load dpt file %s", argv[2]);
    	return -1;
    }
