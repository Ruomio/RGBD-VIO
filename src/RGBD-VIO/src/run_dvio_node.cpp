...
#define NODE_NAME "run_dvio_node"
...
std::queue<sensor_msgs::msg::Image::ConstSharedPtr> dpt_img_buf;
...
sensor_msgs::msg::Image::ConstSharedPtr getDptImage(double timestamp)
{
    ...
    while(true){
        ...
        if(fabs(current_time - timestamp) < 1e-3){
            ...
            RCLCPP_ERROR(rclcpp::get_logger(NODE_NAME), "rvio_syn_node.cpp: what? cannot find syn depth img");
            return NULL;
        }
    }
    return NULL;
...
getMeasurements()
{
    ...
    while (true)
    {
        ...
        if (!(rclcpp::Time(imu_buf.back()->header.stamp).nanoseconds() > rclcpp::Time(feature_buf.front()->header.stamp).nanoseconds() ))
        {
        ...
        if (!(rclcpp::Time(imu_buf.front()->header.stamp).nanoseconds() < rclcpp::Time(feature_buf.front()->header.stamp).nanoseconds()))
        {
            RCLCPP_WARN(rclcpp::get_logger(NODE_NAME), "throw img, only should happen at the beginning");
            feature_buf.pop();
            continue;
        }
        ...
        while (rclcpp::Time(imu_buf.front()->header.stamp).nanoseconds() < rclcpp::Time(img_msg->header.stamp).nanoseconds())
        {
        ...
        IMUs.emplace_back(imu_buf.front());
        if (IMUs.empty())
            RCLCPP_WARN(rclcpp::get_logger(NODE_NAME), "no imu between two image");
...
void dpt_callback(const sensor_msgs::msg::Image::ConstSharedPtr& dpt_img)
{
    ...
    // m_dpt_buf.unlock();
    std::lock_guard<std::mutex> lock(m_dpt_buf);
    dpt_img_buf.push(dpt_img);
}

void imu_callback(const sensor_msgs::msg::Imu::ConstSharedPtr &imu_msg)
{
    if (rclcpp::Time(imu_msg->header.stamp).nanoseconds() <= rclcpp::Time(last_imu_t).nanoseconds())
    {
        RCLCPP_WARN(rclcpp::get_logger(NODE_NAME), "imu message in disorder!");
        return;
    }
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                if (t <= img_t)
                {
                    ...
                    RCLCPP_ASSERT(rclcpp::get_logger(NODE_NAME), dt >= 0);
                    current_time = t;
                ...
                else
                {
                    ...
                    double dt_2 = t - img_t;
                    current_time = img_t;
                    RCLCPP_ASSERT(rclcpp::get_logger(NODE_NAME), dt_1 >= 0);
                    RCLCPP_ASSERT(rclcpp::get_logger(NODE_NAME), dt_2 >= 0);
                    RCLCPP_ASSERT(rclcpp::get_logger(NODE_NAME), dt_1 + dt_2 > 0);
            ...
            RCLCPP_DEBUG(rclcpp::get_logger(NODE_NAME), "processing vision data with stamp %f \n", rclcpp::Time(img_msg->header.stamp).seconds());
            ...
            {
                // cloud[i] is same as cloud.points[i], PCL already opreator[]
            ...
            rvio.processImage_Init_dvio(image, rclcpp::Time(img_msg->header.stamp).seconds());
            ...
            //RCLCPP_ERROR("end: %f, at %f", img_msg->header.stamp.toSec(), rclcpp::Time::now().toSec());
        }
        m_estimator.unlock();
    }
}
...
int main(int argc, char **argv)
{
    ...
    // rclcpp::NodeHandle n("~");
    // rclcpp::console::set_logger_level(ROSCONSOLE_DEFAULT_NAME, rclcpp::console::levels::Debug); // Info
    ...
    // rclcpp::Subscriber sub_restart = n.subscribe("/feature_tracker/restart", 2000, restart_callback);
    // rclcpp::Subscriber sub_relo_points = n.subscribe("/pose_graph/match_points", 2000, relocalization_callback);
