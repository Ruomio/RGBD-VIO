...
bool RVIO::getFloorAndObstacle(const cv::Mat& dpt)
{
	...
	for(int i=0; i<(int)in_pc->points.size(); i++){

		pcl::PointXYZ& pti = in_pc->points[i];
		tf2::Vector3 pii(pti.x, pti.y, pti.z);
	    tf2::Vector3 pww = mCurrPose * pii; // transform from F[frame_count] to world coordinate system
	...
	// double min_z = -1.5;
	   min_z = 1e10;
	   max_z = -1e10;
        for(int i=0; i<(int)in_pc_w->points.size(); i++)
        {
    ...
    Plane::computeByPCL<pcl::PointXYZ>(tmp, indices, nv, nd, 0.1);
    ...
    if((int)indices->indices.size() < 500 || angle < COS5) // NO Floor plane is observed
    ...
    }else // succeed to get a floor plane
    {
        ...
        double sum_z = 0;
        for(int i=0; i<(int)indices->indices.size(); i++)
        {
	    ...
	    {
			tf2::Quaternion tq = mCurrIMUPose.getRotation();
			tf2::Vector3 tt = mCurrIMUPose.getOrigin();
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("rvio_plane"), "rvio_plane.cpp: remove floor cost %f ms", t_fz.toc());
    return true ;
}
