...
bool PlaneCorrectPose::correctPose(const RVIO& estimator)
{
	...
	tf2::Transform inc_pose = prevPose.inverse() * currPose;
	currCorrPose = prevCorrPose * inc_pose;

	if(estimator.bPls[estimator.frame_count] == false){ // no way to update it.
		// RCLCPP_ERROR("what frame_count: %i and no floor plane is founded ", estimator.frame_count);
		return false;
	}
	...
	if(delta_pi.norm() >= 0.5){
		RCLCPP_ERROR(rclcpp::get_logger("plane_correct_pose"), "plane_correct_pose.cpp: large residual : %lf  delta_pi: %f %f %f", residual, delta_pi(0), delta_pi(1), delta_pi(2));
	}else{
		tf2::Vector3 new_pi(Pi(0) + delta_pi(0), Pi(1) + delta_pi(1), Pi(2) + delta_pi(2));
		currCorrPose.setOrigin(new_pi);
	}

	// RCLCPP_DEBUG("Pose_i: %f %f %f, corrected_Pose_i: %f %f %f", Pi(0), Pi(1), Pi(2),
	//		new_pi.getX(), new_pi.getY(), new_pi.getZ());

	// RCLCPP_WARN("plane_correct_pose.cpp: after correct, residual : %lf ", residual);
