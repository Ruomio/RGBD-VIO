...
namespace{
    ...
    void printTF(tf2::Transform& T, string name="")
    {
...
void RVIO::setInputDepthImage(const cv::Mat& dpt_img)
{
    ...
    std::shared_ptr<pcl::PointCloud<pcl::PointXYZI> > tmpPC(new pcl::PointCloud<pcl::PointXYZI>());
    // mPC->clear();
    mDepthPC.swap(tmpPC);
        ...
        if(mDepthPC->width >= 10){
            ...
            if(pointSearchSqrDis[0] < 0.5 && pointSearchInd.size() == 3){
                ...
                // if((query_pt_depth < 0.3 || query_pt_depth > 5)){
                    // RCLCPP_WARN("what? query_pt_depth: %lf minDepth: %lf maxDepth: %lf", query_pt_depth, minDepth, maxDepth);
                // }

                if(query_pt_depth != query_pt_depth){
                    RCLCPP_ERROR(rclcpp::get_logger("rvio"), "what? query_pt_depth is NaN, x1: %f y1: %f z1: %f x2: %f y2: %f z2: %f x3: %f y3: %f z3: %f u: %f v:%f",
                             x1, y1, z1, x2, y2, z2, x3, y3, z3, u, v);
                    RCLCPP_WARN(rclcpp::get_logger("rvio"), "up: %lf down: %lf", up, down);
                }
...
void RVIO::setParameter()
{
    ...
    mTIC = tf2::Transform(tf2::Quaternion(q.x(), q.y(), q.z(), q.w()), tf2::Vector3(tic[0][0], tic[0][1], tic[0][2]));
...
{
    RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "timestamp %lf with feature points %lu", header, image.size());
    ...
    RCLCPP_INFO(rclcpp::get_logger("rvio"), "handle frame at timstamp %lf is a %s", header, marginalization_flag ? "Non-keyframe" : "Keyframe");
    RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "Solving %d", frame_count);
    RCLCPP_INFO(rclcpp::get_logger("rvio"), "timestamp %lf number of feature: %d", header, f_manager.getFeatureCount());
    ...
    if(solver_flag == INITIAL){
        ...
        if(frame_count == WN){
            ...
            if(result) // succeed to initialize
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("rvio"), "Initialization finish!");
                showStatus();
                ...
                RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "RVIO.cpp: failed to initialize structure");
                slideWindow();
...
void RVIO::solveGyroscopeBias(map<double, ImageFrame> &all_image_frame, Vector3d* Bgs)
{
    ...
    RCLCPP_WARN_STREAM(rclcpp::get_logger("rvio"), "gyroscope bias initial calibration " << delta_bg.transpose());
...
void RVIO::solveMono()
{
    ...
    {
        ceres::Manifold *local_parameterization = new PoseLocalPrameterization();
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        {
            RCLCPP_DEBUG("fix extinsic param");
            problem.SetParameterBlockConstant(para_Ex_Pose[i]);
        }
        else
            RCLCPP_DEBUG("estimate extinsic param");
    }
    ...
    RCLCPP_DEBUG("visual measurement count: %d", f_m_cnt);
    RCLCPP_DEBUG("prepare for ceres: %f", t_prepare.toc());
    ...
    RCLCPP_DEBUG("Iterations : %d", static_cast<int>(summary.iterations.size()));
    RCLCPP_DEBUG("solver costs: %f", t_solver.toc());
    // exit(0);
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        RCLCPP_DEBUG("pre marginalization %f ms", t_pre_margin.toc());

        TicToc t_margin;
        marginalization_info->marginalize();
        RCLCPP_DEBUG("marginalization %f ms", t_margin.toc());
    ...
    else
    {
            ...
            std::count(std::begin(last_marginalization_parameter_blocks), std::end(last_marginalization_parameter_blocks), para_Pose[WINDOW_SIZE - 1]))
        {
            ...
            if (last_marginalization_info)
            {
                ...
                for (int i = 0; i < static_cast<int>(last_marginalization_parameter_blocks.size()); i++)
                {
                    RCLCPP_ASSERT(last_marginalization_parameter_blocks[i] != para_SpeedBias[WINDOW_SIZE - 1]);
            ...
            TicToc t_pre_margin;
            RCLCPP_DEBUG("begin marginalization");
            marginalization_info->preMarginalize();
            RCLCPP_DEBUG("end pre marginalization, %f ms", t_pre_margin.toc());

            TicToc t_margin;
            RCLCPP_DEBUG("begin marginalization");
            marginalization_info->marginalize();
            RCLCPP_DEBUG("end marginalization, %f ms", t_margin.toc());
    ...
    RCLCPP_DEBUG("whole marginalization costs: %f", t_whole_marginalization.toc());

    RCLCPP_DEBUG("whole time for ceres: %f", t_whole.toc());
}
*/
...
void RVIO::solveMono()
{
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        if (!ESTIMATE_EXTRINSIC)
        {
            RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "fix extinsic param");
            problem.SetParameterBlockConstant(para_Ex_Pose[i]);
        }
        else
            RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "estimate extinsic param");
    }
    ...
    ceres::Solve(options, &problem, &summary);
    //cout << summary.BriefReport() << endl;
    RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "Iterations : %d", static_cast<int>(summary.iterations.size()));
    ...
    else
    {
            ...
            std::count(std::begin(last_marginalization_parameter_blocks), std::end(last_marginalization_parameter_blocks), para_Pose[WINDOW_SIZE - 1]))
        {
            ...
            if (last_marginalization_info)
            {
                ...
                for (int i = 0; i < static_cast<int>(last_marginalization_parameter_blocks.size()); i++)
                {
                    RCLCPP_ASSERT(rclcpp::get_logger("rvio"), last_marginalization_parameter_blocks[i] != para_SpeedBias[WINDOW_SIZE - 1]);
                    if (last_marginalization_parameter_blocks[i] == para_Pose[WINDOW_SIZE - 1])
                        drop_set.push_back(i);
                }
                // construct new marginlization_factor
...
void RVIO::solveOdometry()
{
    ...
    // RCLCPP_DEBUG("RVIO.cpp: now frame_count = %d", frame_count);

    // add pose
    for(int i=0; i<= frame_count; i++){
        ceres::Manifold *local_param = new PoseLocalPrameterization();
    ...
    for(auto &it_per_id : f_manager.feature){
        ...
        if(it_per_id.estimated_depth > 0 && it_per_id.solve_flag != 2){
            ...
            if(feature_index >= NUM_OF_FEAT){
                RCLCPP_ERROR(rclcpp::get_logger("rvio"), "RVIO.cpp: feature_index = %d larger than %d ", feature_index, NUM_OF_FEAT);
                continue;
            }
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "RVIO.cpp: before optimization, %d features have been used with %d constrints!", cnt_used_features, f_m_cnt);
    // optimize it
    ceres::Solver::Options options;
    options.linear_solver_type = ceres::DENSE_SCHUR;
    //options.num_threads = 2;
    ...
    // cout << summary.BriefReport() << endl;
    RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "Iterations : %d", static_cast<int>(summary.iterations.size()));
    RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "solver costs: %f", t_solver.toc());
    ...
    if(marginalization_flag == MARGIN_OLD){
        ...
        if(it_per_id.estimated_depth > 0 && it_per_id.solve_flag != 2){
            ...
            if(imu_i == 0){ // marginalized the node with depth
                ...
                RCLCPP_ERROR(rclcpp::get_logger("rvio"), "now should not arrive here!");
            }
        ...
        TicToc t_pre_margin;
        marginalization_info->preMarginalize();
        RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "RVIO.cpp: pre marginalization: %f ms", t_pre_margin.toc());

        TicToc t_margin;
        marginalization_info->marginalize();
        RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "RVIO.cpp: marginalization %f ms", t_margin.toc());
        ...
        if(last_marginalization_info &&
            ...
            if(last_marginalization_info && last_marginalization_info->valid){
                vector<int> drop_set;
                for(int i=0; i<last_marginalization_parameter_blocks.size(); i++){
                    RCLCPP_ASSERT(rclcpp::get_logger("rvio"), last_marginalization_parameter_blocks[i] != para_SpeedBias[WN-1]);
...
void RVIO::afterOptimize()
{
    ...
    {
        RCLCPP_DEBUG(rclcpp::get_logger("rvio"), "euler singular point!");
    ...
    mCurrIMUPose = tf2::Transform(tf2::Quaternion(q.x(), q.y(), q.z(), q.w()), tf2::Vector3(Ps[frame_count][0], Ps[frame_count][1], Ps[frame_count][2]));
    RCLCPP_INFO(rclcpp::get_logger("rvio"), "RVIO.cpp: afterOptimize at frame_count = %d", frame_count);
