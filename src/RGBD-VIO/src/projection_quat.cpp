...
Eigen::Matrix<double, 3, 2> Unit3::getBasis(Eigen::Matrix62* H) const
{
    if(B_ && !H)
        return *B_;
    if(B_ && H && H_B_)
...
bool PoseLocalPrameterization::PlusJacobian(const double *x, double *jacobian) const
{
...
bool PoseLocalPrameterization::RightMultiplyByPlusJacobian(const double *x, const int num_rows, const double *ambient_matrix, double *tangent_matrix) const
{
    // 1. get plus_jacobian
    ...
    Eigen::Map<const Eigen::Matrix<double, Eigen::Dynamic, 7, Eigen::RowMajor>>
        ambient_mat(ambient_matrix, num_rows, 7);
    Eigen::Map<Eigen::Matrix<double, Eigen::Dynamic, 6, Eigen::RowMajor>>
        tangent_mat(tangent_matrix, num_rows, 6);
...
bool PoseLocalPrameterization::Minus(const double *y, const double *x, double *y_minus_x) const
{
...
bool PoseLocalPrameterization::MinusJacobian(const double *x, double *jacobian) const
{
