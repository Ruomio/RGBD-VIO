								...
								vector<SFMFeature> &sfm_f)
{
	...
	if (int(pts_2_vector.size()) < 15)
	{
		...
		if (int(pts_2_vector.size()) < 10){
			printf("initial_sfm.cpp: return false since only %d features are found", (int)pts_2_vector.size());
			return false;
		}
	}
	...
	for (int j = 0; j < feature_num; j++)
	{
		...
		if (has_0 && has_1)
		{
			...
			if(point0.z() > 0.1){
				...
				if (residual.norm() < 1.0/FOCAL_LENGTH){
					sfm_f[j].state = true;
 ...
 // this is the depth version
	...
	for (int j = 0; j < feature_num; j++)
	{
		...
		if ((int)sfm_f[j].observation.size() >= 2)
		{
			...
			if (residual.norm() < 1.0/FOCAL_LENGTH) {//reprojection error
				sfm_f[j].state = true;
				sfm_f[j].position[0] = point_3d(0);
				sfm_f[j].position[1] = point_3d(1);
				sfm_f[j].position[2] = point_3d(2);
...
*/
	...
	ceres::QuaternionManifold* local_parameterization = new ceres::QuaternionManifold();
