...
bool MotionEstimator::solveRelativeRT(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation, Vector3d &Translation)
{
    if (corres.size() >= 15)
    {
        ...
        if(inlier_cnt > 12){
            ...
            RCLCPP_WARN(rclcpp::get_logger("solve_5pts"), "---------------5points----------------");
            RCLCPP_WARN(rclcpp::get_logger("solve_5pts"), "input points %d inliers %d", (int)ll.size(), inlier_cnt);
...
bool MotionEstimator::solveRelativeHybrid(const vector<pair<Vector3d, Vector3d>> &corres, Matrix3d &Rotation, Vector3d &Translation, vector<Vector3d>* pcov)
{
    ...
    if (corres.size() >= 15)
    {
        ...
        if(inlier_cnt > 12 && inliers.size() >= 5){
            ...
            RCLCPP_WARN(rclcpp::get_logger("solve_5pts"), "---------------5points----------------");
            RCLCPP_WARN(rclcpp::get_logger("solve_5pts"), "input points %d 2D inliers %d 3D inliers %d", (int)ll.size(), (int)inlier_cnt, (int)inliers.size());
