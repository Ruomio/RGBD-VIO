...
// #include <pcl_ros/kdtree/kdtree_flann.hpp>
...
public:
   ...
   // depth plane related
    ...
    std::shared_ptr<pcl::PointCloud<pcl::PointXYZI> > mDepthPC;
    std::shared_ptr<pcl::KdTreeFLANN<pcl::PointXYZI> > mKDTree;

    double mFloorZ;
    double mFloorRange;
    std::shared_ptr<pcl::PointCloud<pcl::PointXYZI> > mPCNoFloor;
    std::shared_ptr<pcl::PointCloud<pcl::PointXYZ> > mPCFloor;
    ...
    vector<tf2::Transform> mFtTransCurr; // record cam's pose when the features are first observed
    vector<tf2::Transform> mFtTransLast; // record cam's pose when the features are first observed
    ...
    tf2::Transform mInitCamPose; // Initial campose
    tf2::Transform mLastPose; // camera pose for the mImgPTLast
    tf2::Transform mCurrPose; // camera pose for the mImgPTCurr
    tf2::Transform mCurrIMUPose; // current IMU pose
    tf2::Transform mTIC;  // transform from imu to camera
    ...
    nav_msgs::msg::Path m_gt_path;  // groundtruth path
    nav_msgs::msg::Path m_est_path; // estimated path
public:
