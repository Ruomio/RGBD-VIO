...
using namespace Eigen;
 ...
 {
 	RCLCPP_DEBUG(rclcpp::get_logger("rvio_dvio"), "timestamp %lf with feature points %lu", header, image.size());
    ...
    RCLCPP_INFO(rclcpp::get_logger("rvio_dvio"), "handle frame at timstamp %lf is a %s", header, marginalization_flag ? "Non-keyframe" : "Keyframe");
    RCLCPP_DEBUG(rclcpp::get_logger("rvio_dvio"), "Solving %d", frame_count);
    RCLCPP_INFO(rclcpp::get_logger("rvio_dvio"), "timestamp %lf number of feature: %d", header, f_manager.getFeatureCount());
    ...
    if(solver_flag == INITIAL){
        ...
        if(frame_count == WN){
            ...
            if(result) // succeed to initialize
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("rvio_dvio"), "Initialization finish!");
                showStatus();
                ...
                RCLCPP_DEBUG(rclcpp::get_logger("rvio_dvio"), "RVIO.cpp: failed to initialize structure");
                slideWindow();
...
void RVIO::slideWindow_dvio()
{
    if(marginalization_flag == MARGIN_OLD){
        ...
        if(frame_count == WN){
            ...
            for(int i=0; i<(int)dt_buf[WN].size(); i++){
...
void RVIO::solveOdometry_dvio()
{
    ...
    for(auto &it_per_id : f_manager.feature){
        ...
        if(it_per_id.estimated_depth > 0 && it_per_id.solve_flag != 2){
            ...
            if(feature_index >= NUM_OF_FEAT){
                RCLCPP_ERROR(rclcpp::get_logger("rvio_dvio"), "rvio_dvio.cpp: feature_index = %d larger than %d ", feature_index, NUM_OF_FEAT);
                continue;
            }
            ...
            for(int shift=0; shift<(int)it_per_id.feature_per_frame.size(); shift++){
            ...
            for(int shift = 1; shift < (int)it_per_id.feature_per_frame.size(); shift++){
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("rvio_dvio"), "rvio_dvio.cpp: before optimization, %d features have been used with %d constrints!", cnt_used_features, f_m_cnt);
    // optimize it
    ...
    RCLCPP_DEBUG(rclcpp::get_logger("rvio_dvio"), "rvio_dvio.cpp: Iterations : %d", static_cast<int>(summary.iterations.size()));
    RCLCPP_DEBUG(rclcpp::get_logger("rvio_dvio"), "rvio_dvio.cpp: solver costs: %f", t_solver.toc());
    ...
    if(marginalization_flag == MARGIN_OLD){
        ...
        if(it_per_id.estimated_depth > 0 && it_per_id.solve_flag != 2){
            ...
            if(imu_i == 0){ // marginalized the node with depth
                for(int imu_j=0; imu_j<(int)it_per_id.feature_per_frame.size(); imu_j++){
            	...
            	// RCLCPP_ERROR("now should not arrive here!");
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("rvio_dvio"), "rvio_dvio.cpp: pre marginalization: %f ms", t_pre_margin.toc());

        TicToc t_margin;
        marginalization_info->marginalize();
        RCLCPP_DEBUG(rclcpp::get_logger("rvio_dvio"), "rvio_dvio.cpp: marginalization %f ms", t_margin.toc());
        ...
        if(last_marginalization_info &&
            ...
            if(last_marginalization_info && last_marginalization_info->valid){
                vector<int> drop_set;
                for(int i=0; i<(int)last_marginalization_parameter_blocks.size(); i++){
                    RCLCPP_ASSERT(rclcpp::get_logger("rvio_dvio"), last_marginalization_parameter_blocks[i] != para_SpeedBias[WN-1]);
